...
bool Grid::RemoveObjectFromCell(const CellPosition & pos)//M
{
	if (pos.IsValidCell()) // Check if valid position
	{
...
Card* Grid::CheckACard(CellPosition CellPos)//M
//to check if a cell has card or not and Return it
{
	if (CellList[CellPos.VCell()][CellPos.HCell()]->HasCard())
		return dynamic_cast<Card *>(CellList[CellPos.VCell()][CellPos.HCell()]->GetGameObject());
	return NULL;
}

void Grid::set_New_Info_Players()//M
{
	
	for (int i = 0; i < MaxPlayerCount; i++)
	{
		
		this->UpdatePlayerCell(PlayerList[i], CellList[8][0]->GetCellPosition());
	...
	// Restarts players' positions, wallets, 
}
...
void Grid::SaveAll(ofstream & OutFile, GameObjectEnum g)//M
{
